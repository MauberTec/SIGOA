CREATE PROCEDURE [dbo].[STP_SEL_OBJETO_GRUPO_OBJETO_VARIAVEIS_VALORES] 
@obj_id int = 3,
@ord_id int = -1
as 
begin 
	...
	if (@ord_id >0)
	begin
		 execute  dbo.STP_SEL_INSPECAO_GRUPO_OBJETO_VARIAVEIS_VALORES @ord_id
		 return 1;
    end;
else
begin
			...
			from (
					...
					union
						select 
						    ...
						    isnull(ovv.ogi_id_caracterizacao_situacao,'') as ogi_id_caracterizacao_situacao,
						    ...
						    isnull(ovv_observacoes_gerais,'') as ovv_observacoes_gerais, 
						    isnull(ovv.tpu_id,'') as tpu_id, 
					      --  isnull(ovv.ovv_descricao_servico,'') as tpu_descricao,
							...
							isnull(ovv_unidade_servico,'') as uni_unidade,
						    isnull(ovv_tpu_quantidade,'') as ovv_tpu_quantidade
					from tab_objetos obj
						...
						left join  tab_objeto_grupo_objeto_variaveis_valores ovv on ovv.ogv_id = ogv.ogv_id and ovv.obj_id=obj.obj_id and ovv.ovv_deletado is null--or ovv.ogv_id is null 
						...
						left join dbo.tab_objeto_grupo_objetos_variaveis_itens ogi1 on ogi1.ogi_id = ovv.ogi_id_caracterizacao_situacao and ogi1.ogi_ativo = 1 and ogi1.ogi_deletado is null and ogi1.ogv_id=0
						left join dbo.tab_tpu_precos_unitarios tpu on tpu.tpu_id = ovv.tpu_id 
						left join dbo.tab_unidades_medida uni on uni.uni_id = tpu.uni_id

						left join dbo.tab_objeto_atributos_valores atv on atv.obj_id = obj.obj_id and atv.atr_id = 129
						left join dbo.tab_atributo_itens ati on ati.ati_id = atv.ati_id 

						 left join dbo.tab_objeto_conserva_politica ocp on ocp.ogi_id_caracterizacao_situacao = ovv.ogi_id_caracterizacao_situacao and ocp.ogv_id= ovv.ogv_id and ocp_deletado is null and ocp_ativo = 1
